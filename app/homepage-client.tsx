    ...
    <div className="space-y-16 pb-16 animate-in fade-in duration-700">
        ...
        <div className="container mx-auto px-4 lg:px-8 py-20 lg:py-32 relative z-10">
          <div className="max-w-2xl space-y-6">
            ...
            <div className="pt-4 flex gap-4">
              <Link href="/products" className="h-14 px-8 bg-background text-foreground rounded-full font-black flex items-center justify-center hover:scale-105 transition-transform shadow-xl cursor-pointer">
      ...
      <div className="container mx-auto px-4 lg:px-8 space-y-16">
        ...
        <section className="space-y-6">
          <h2 className="text-3xl font-black tracking-tight">{t.home.categories}</h2>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {["Electronics", "Fashion", "Home & Living", "Furniture", "Beauty", "Sports"].map((cat) => (
              <div key={cat} className="aspect-square rounded-[2rem] bg-muted/50 border hover:bg-primary/5 hover:border-primary/20 transition-colors flex flex-col items-center justify-center gap-3 cursor-pointer group">
        ...
        {officialProducts.length > 0 && (
          <section className="space-y-6 p-8 rounded-[3rem] bg-gradient-to-r from-amber-500/10 to-rose-500/10 border-2 border-amber-500/20">
            <div className="flex items-center gap-3">
              <ShieldCheck className="h-8 w-8 text-amber-600" />
              <h2 className="text-3xl font-black tracking-tight text-amber-700 dark:text-amber-500">{t.home.officialStores}</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {officialProducts.map(product => (
                <Link key={product.id} href={`/products/${product.id}`} className="group bg-card rounded-[2rem] border overflow-hidden hover:shadow-xl transition-all cursor-pointer">
        ...
        <section className="space-y-6">
          <div className="flex justify-between items-end">
            <h2 className="text-3xl font-black tracking-tight">{t.home.dailyDiscover}</h2>
            <Link href="/products" className="text-primary font-bold hover:underline">{t.common.view} All</Link>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {featuredProducts.map(product => (
              <Link key={product.id} href={`/products/${product.id}`} className="group bg-card rounded-[2rem] border overflow-hidden hover:shadow-xl transition-all cursor-pointer">
                ...
                <div className="p-5 space-y-2">
                  <h3 className="font-bold line-clamp-2 text-sm group-hover:text-primary transition-colors leading-snug">{product.name}</h3>
