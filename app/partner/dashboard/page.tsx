...
export default function PartnerDashboardPage() {
  ...
  const smartAdvice = useMemo(() => {
    ...
    if (myProducts.length === 0) {
      advice.push({ title: "เริ่มการขายก้าวแรก", desc: "เพิ่มสินค้าชิ้นแรกของคุณวันนี้! แนะนำเป็น Gadget ยอดนิยม เช่น หูฟังไร้สาย หรือ Smart Watch เพื่อดึงดูดลูกค้า", icon: Zap, color: "bg-amber-500" });
    }
    const lowStock = myProducts.filter((p: any) => p.stock < 5 && p.stock > 0);
    if (lowStock.length > 0) {
      advice.push({ title: "สต็อกใกล้หมด!", desc: `สินค้า ${lowStock.length} รายการของคุณกำลังจะหมด (เหลือต่ำกว่า 5 ชิ้น) เติมสต็อกตอนนี้เพื่อยอดขายที่ไม่สะดุด`, icon: AlertTriangle, color: "bg-rose-500" });
    }
    const flatPricing = myProducts.filter((p: any) => p.price % 100 === 0);
    if (flatPricing.length > 0) {
      advice.push({ title: "จิตวิทยาการตั้งราคา", desc: "ลองปรับราคาให้ลงท้ายด้วยเลข 9 (เช่น ฿990 แทน ฿1,000) ผลวิจัยชี้ว่าช่วยเพิ่มอัตราการสั่งซื้อได้ถึง 15%", icon: Tag, color: "bg-blue-500" });
    }
  ...
  return (
    <div className="p-4 lg:p-8 space-y-8 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2"><button onClick={() => { setNewProduct({ id: "", name: "", price: 0, category: "Electronics", stock: 0, image: "", description: "" }); setShowAddModal(true); }} className="h-10 px-4 rounded-xl bg-primary text-primary-foreground flex items-center gap-2 font-black text-[10px] uppercase tracking-widest hover:opacity-90 transition-all shadow-lg shadow-primary/20 cursor-pointer"><Plus className="h-3.5 w-3.5" /> {t.dashboard.addProduct}</button></div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-card border-2 border-primary/5 rounded-4xl p-6 shadow-xl shadow-primary/5 space-y-4">
            ...
            <div><p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">{stat.label}</p><h3 className="text-3xl font-black tracking-tighter">{stat.value}</h3><p className="text-[10px] font-black text-emerald-500">{stat.change}</p></div>
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
                ...
                <tbody className="divide-y">
                  {myOrders.length > 0 ? myOrders.map((order: any) => (
                    <tr key={order.order_id || order.id} className="hover:bg-muted/30 transition-colors">
                      <td className="px-8 py-6"><div className="space-y-1"><p className="font-black text-sm">{order.order_id || order.id}</p><p className="text-[10px] font-bold text-muted-foreground">{new Date(order.timestamp || order.createdAt).toLocaleDateString()}</p></div></td>
                      <td className="px-8 py-6 font-black text-sm">{formatCurrency(order.total_price || order.totalAmount)}</td>
                      <td className="px-8 py-6"><select className={cn("text-[10px] font-black uppercase tracking-tight bg-background border-2 rounded-xl px-3 py-2 outline-none cursor-pointer transition-all", order.status.toUpperCase() === "PENDING" ? "border-amber-500/20 text-amber-600 focus:border-amber-500" : order.status.toUpperCase() === "DELIVERED" ? "border-emerald-500/20 text-emerald-600 focus:border-emerald-500" : "border-primary/20 text-primary focus:border-primary")} value={order.status.toUpperCase()} onChange={async (e) => { const newStatus = e.target.value; const res = await fetch("/api/orders/status", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: order.order_id || order.id, status: newStatus }) }); if (res.ok) { fetchAllData(); toast.success("อัปเดตสถานะออเดอร์เรียบร้อย"); } }}><option value="PENDING">{t.orders.status.pending}</option><option value="SHIPPED">{t.orders.status.shipped}</option><option value="DELIVERED">{t.orders.status.delivered}</option><option value="CANCELLED">{t.orders.status.cancelled}</option></select></td>
                    </tr>
                  )) : <tr><td colSpan={3} className="px-8 py-12 text-center text-muted-foreground font-bold italic uppercase text-xs tracking-widest">ยังไม่มีรายการสั่งซื้อ / No orders yet</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="space-y-6">
           <div className="bg-gradient-to-br from-indigo-500 via-purple-600 to-primary rounded-4xl p-8 text-white shadow-xl shadow-primary/20 space-y-6 animate-in zoom-in duration-500">
             <div className="flex items-center gap-3"><div className="p-2 bg-white/20 rounded-xl backdrop-blur-sm shadow-inner"><Sparkles className="h-5 w-5" /></div><h3 className="font-black uppercase tracking-widest text-sm">Smart Advice by AI</h3></div>
             ...
             <div className="pt-2"><p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 text-center italic">Calculated in real-time by Pickpock AI</p></div>
           </div>
           <div className="bg-card border-2 border-primary/10 rounded-4xl p-8 shadow-xl shadow-primary/5 space-y-6"><h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground">{t.dashboard.inventoryAlerts}</h3><div className="space-y-4">{myProducts.filter((p: any) => p.stock < 10).slice(0, 5).map((p: any) => (<div key={p.product_id || p.id} className="flex items-center justify-between p-4 rounded-2xl bg-muted/50 border border-transparent hover:border-amber-500/20 transition-all group"><div className="flex items-center gap-3"><div className="p-2 rounded-lg bg-white shadow-sm group-hover:scale-110 transition-transform"><Package className="h-4 w-4 text-amber-500" /></div><p className="text-[10px] font-black uppercase truncate max-w-[100px]">{p.name}</p></div><span className="text-[10px] font-black text-amber-600 bg-amber-500/10 px-2 py-1 rounded-lg">{p.stock} {t.product.quantity}</span></div>))}{myProducts.filter((p: any) => p.stock < 10).length === 0 && (<p className="text-center text-[10px] font-bold text-muted-foreground italic">สถานะสต็อกสินค้าปกติ / Stock is healthy</p>)}</div></div>
        </div>
      </div>
      ...
      {showEditStoreModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-card w-full max-w-lg rounded-4xl border-2 border-primary/20 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleEditStore} className="p-8 space-y-6"><div className="space-y-4"><div className="space-y-2"><label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t.dashboard.storeName}</label><input type="text" required className="w-full px-6 py-4 rounded-2xl bg-muted/50 border-2 border-transparent focus:bg-background focus:border-primary/20 outline-none transition-all font-bold" value={editStoreData.name} onChange={(e) => setEditStoreData({...editStoreData, name: e.target.value})} /></div><div className="space-y-2"><label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">{t.dashboard.storeDesc}</label><textarea required rows={4} className="w-full px-6 py-4 rounded-2xl bg-muted/50 border-2 border-transparent focus:bg-background focus:border-primary/20 outline-none transition-all font-bold resize-none" value={editStoreData.description} onChange={(e) => setEditStoreData({...editStoreData, description: e.target.value})} /></div></div><button type="submit" className="w-full h-16 rounded-2xl bg-primary text-primary-foreground font-black text-lg shadow-xl shadow-primary/20 hover:opacity-90 active:scale-95 transition-all flex items-center justify-center gap-3 cursor-pointer">{t.dashboard.saveChanges}</button></form>
          </div>
        </div>
      )}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-2"><input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" id="product-image-upload" disabled={isUploading} /><label htmlFor="product-image-upload" className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary/10 text-primary text-[10px] font-black uppercase tracking-widest hover:bg-primary/20 transition-all cursor-pointer disabled:opacity-50"><ImageIcon className="h-3.5 w-3.5" />{isUploading ? t.dashboard.uploading : t.dashboard.selectPhoto}</label><p className="text-[10px] text-muted-foreground font-medium italic leading-tight">{t.dashboard.photoDesc}</p></div>
                  </div>
                </div>
